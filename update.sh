...
for file in box-session.sh bpython colors.sh dbus-env opera pipdc rx show-sizes stg-flush title xxx check-mail git-pull-all urxvt-clipboard fall stg-push-current menu.sh menu.lst dmenu_path startmutt.sh vial svial external-monitor; do
    cp ~/bin/$file bin/
done
